			...
			return (
				<div className={`ev ${(c.type) && 'label_' + c.type}`} onClick={() => props.fn(c)}>
					<div className="ev-desc">
						<b style={{ fontWeight: 'bolder', fontSize: '16px', textTransform: 'uppercase', fontFamily:'info' }}>SEANCE : {c.content}</b><br />
						<b style={{  fontSize: '12px', textTransform: 'uppercase', fontFamily:'info' }}>candidat : {c.candidat}</b> <br />
						<b style={{  fontSize: '12px', textTransform: 'uppercase', fontFamily:'info' }}>employé : {c.employe}</b>
...
export default function AdminCalendrier() {
	...
	useEffect(() => {
		let unmounted = false
		if (!unmounted) {
			Axios.get('http://localhost:5000/events').then(
				(data) => {
